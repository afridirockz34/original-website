...
if (isset($_POST['submit'])) {
    ...
    // --- 2. INPUT SANITIZATION AND INITIALIZATION ---
    // Sanitize all user-provided data BEFORE validation or use.
    $error1 = "";
    
    // Sanitize and define all POST variables used in the script
    $phpPicVal1  = htmlspecialchars($_POST['phpCalVal1'] ?? '');
    $calValInput = htmlspecialchars($_POST['calVal1'] ?? '');
    $company1    = htmlspecialchars($_POST['company1'] ?? '');
    $website1    = htmlspecialchars($_POST['website1'] ?? '');
    $email1      = htmlspecialchars($_POST['email1'] ?? '');
    $username1   = htmlspecialchars($_POST['username1'] ?? '');
    $phone1      = htmlspecialchars($_POST['phone1'] ?? '');
    
    // Get values from select fields, sanitize and provide defaults if not set
    $accountType = htmlspecialchars($_POST['account-type'] ?? 'no'); 
    $inquiryType = htmlspecialchars($_POST['inquiry-type'] ?? 'more-calls'); 
    $accountID   = htmlspecialchars($_POST['accountid'] ?? 'N/A');
    ...
    // --- 4. VALIDATION CHECKS (Using sanitized variables) ---
    ...
    // Email check (Combined empty and format validation)
    if (empty($email1)) {
        $error1.= '&nbsp; <i style="font-size:8px; position:relative; top:-2px;" class="fas fa-circle"></i>&nbsp; Please enter your email address!<br>';
    ...
    // Full name check
    if (empty($username1)) {
        $error1.= '&nbsp; <i style="font-size:8px; position:relative; top:-2px;" class="fas fa-circle"></i>&nbsp; Please enter your full name!<br>';
    }

    // Calculation (Captcha) check
    if (empty($calValInput)) {
        $error1.= '&nbsp; <i style="font-size:8px; position:relative; top:-2px;" class="fas fa-circle"></i>&nbsp; Please complete the calculation!<br>';
    } else if ($calValInput != $calValue1) {
        // This handles both the wrong answer AND the case where $phpPicVal1 was an unknown image.
        $error1.= '&nbsp; <i style="font-size:8px; position:relative; top:-2px;" class="fas fa-circle"></i>&nbsp; Calculated value is wrong!<br>';
    }

    // --- 5. ERROR HANDLING AND SUCCESS ---
    
    if ($error1 != "") {
        // Errors exist, display them and jump to the form section
        $error1 = '<h4 style="font-weight:16px; font-weight:bold">There were error(s) in your form:</h4>' . $error1; 
        
        // Use echo for clean separation of PHP and JavaScript
        echo '<script>window.location.hash="#check-eligibility"</script>';
    } else {
        // No errors, proceed to sending emails

        // Prepare variables for email
        $userName = ucwords($username1);
...
<a style="color:black; text-decoration:none; font-size:14px;" href="https://6ixdevelopers.com/privacy-policy">Privacy Policy</a>&nbsp;  | &nbsp;<a style="color:black; text-decoration:none; font-size:14px;" href="https://6ixdevelopers.com/terms-and-conditions">Terms & Conditions</a>&nbsp; | &nbsp;<a style="color:black; text-decoration:none; font-size:14px;" href="https://6ixdevelopers.com/sitemap">Sitemap</a>
        ...
        // NOTE: $content2 is still mostly the same as your original, 
        // using the sanitized $userName.
        ...
        $emailTo3 = "faheem-afridi@live.com, musab@6ixdevelopers.com";
        $subject3 = "New Google Ads Credit Request From " . $username1 . " 6ixdevelopers - PPC-google-ads-management-toronto";
        
        // IMPORTANT: The variables $date, $lead_source, and $utm_source are now defined at the top!
        ...
        <tr style="border-collapse:collapse; text-align:left;">
        <td style="background-color:#F2F3F4; border-collapse:collapse; border: 1px solid grey; font-weight:bold; width:25%;">Name</td>
        <td style="background-color:#F2F3F4; border-collapse:collapse; border: 1px solid grey; width:25%;">' . $username1 . '</td>
        <td style="background-color:#F2F3F4; border-collapse:collapse; border: 1px solid grey; font-weight:bold; width:25%;">Email</td>
        <td style="background-color:#F2F3F4; border-collapse:collapse; border: 1px solid grey; width:25%;">' . $email1 . '</td>
        </tr>
        <tr style="border-collapse:collapse; text-align:left;">
        <td style="border-collapse:collapse; border: 1px solid grey; font-weight:bold; width:25%;">Phone</td>
        <td style="border-collapse:collapse; border: 1px solid grey; width:25%;">' . $phone1 . '</td>
        <td style="border-collapse:collapse; border: 1px solid grey; font-weight:bold; width:25%;">Current Website</td>
        <td style="border-collapse:collapse; border: 1px solid grey; width:25%;">' . $website1 . '</td>
        </tr>
        <tr style="border-collapse:collapse; text-align:left;">
        ...
        <td style="background-color:#F2F3F4; border-collapse:collapse; border: 1px solid grey; width:25%;">' . $company1 . '</td>
        ...
        <td style="background-color:#F2F3F4; border-collapse:collapse; border: 1px solid grey; width:25%;">' . $accountType . '</td>
        </tr>
        <tr style="border-collapse:collapse; text-align:left;">
        <td style="border-collapse:collapse; border: 1px solid grey; font-weight:bold; width:25%;">Account ID</td>
        <td style="border-collapse:collapse; border: 1px solid grey; width:25%;">' . $accountID . '</td>
        <td style="border-collapse:collapse; border: 1px solid grey; font-weight:bold; width:25%;">Inquiry Type</td>
        <td style="border-collapse:collapse; border: 1px solid grey; width:25%;">' . $inquiryType . '</td>
        </tr>
        <tr style="border-collapse:collapse; text-align:left;">
            <td style="background-color:#F2F3F4; border-collapse:collapse; border: 1px solid grey; font-weight:bold; width:25%;">Lead Source</td>
            <td style="background-color:#F2F3F4; border-collapse:collapse; border: 1px solid grey; width:25%;">'. $lead_source .'</td>
            <td style="background-color:#F2F3F4; border-collapse:collapse; border: 1px solid grey; font-weight:bold; width:25%;">UTM Source (if any):</td>
            <td style="background-color:#F2F3F4; border-collapse:collapse; border: 1 tag solid grey; width:25%;">'. $utm_source .'</td>
        </tr>
        </table>
        <p><br></p><p></p><br>';
        ...
        // Sending the second email (The one you are having trouble with)
        if (mail($emailTo3, $subject3, $content3, $headers3)) {
            ...
            // Mail function failed to execute
            $error1 = '<p>Due to a problem on the server, we could not send your message. Please try again later. 6ixdevelopers team&reg;.</p>';
        }
    }
}
...
if (isset($_POST['submit-audit'])) {
    ...
    if ($error2 != "") {
        $error2 = '<h4 style="font-weight:16px; font-weight:bold">There were error(s) in your form:</h4>' . $error2; ?><script>window.location.hash="#check-eligibility"</script><?php
    } else {
...
<a style="color:black; text-decoration:none; font-size:14px;" href="https://6ixdevelopers.com/privacy-policy">Privacy Policy</a>&nbsp;  | &nbsp;<a style="color:black; text-decoration:none; font-size:14px;" href="https://6ixdevelopers.com/terms-and-conditions">Terms & Conditions</a>&nbsp; | &nbsp;<a style="color:black; text-decoration:none; font-size:14px;" href="https://6ixdevelopers.com/sitemap">Sitemap</a>
        ...
        $emailTo5 = "faheem-afridi@live.com, musab@6ixdevelopers.com";
       ...
       <script type="application/ld+json">
{
  ...
  "@graph": [
    {
      "@type": "WebPage",
      "@id": "https://6ixdevelopers.com/ppc-google-ads-management-toronto#webpage",
      "url": "https://6ixdevelopers.com/ppc-google-ads-management-toronto",
      "name": "Google Ads Agency Toronto | 6ix Developers LTD.",
      "description": "Toronto’s top-rated Google Ads Agency specializing in Google Ads PPC. Managing $15M+ per month on Google Ads Spend. 4.9★ Google Rating. Call us today.",
      "isPartOf": {
    ...
    {
      "@type": "BreadcrumbList",
      "@id": "https://6ixdevelopers.com/ppc-google-ads-management-toronto#breadcrumb",
      "itemListElement": [
        {
          "@type": "ListItem",
          "position": 1,
        ...
        },
        {
          "@type": "ListItem",
          "position": 2,
          "name": "Google Ads Agency Toronto",
          "item": "https://6ixdevelopers.com/ppc-google-ads-management-toronto"
        }
      ]
    },
...
/* Selected state */
.gd-offer-card--selected {
  border-color: #2563eb;
  box-shadow: 0 18px 40px rgba(37, 99, 235, 0.25);
}

.gd-offer-card--selected .gd-offer-check {
  background: #2563eb;
  border-color: #2563eb;
}
...
.gd-offers-footer-text h3 {
  margin: 0 0 4px;
  font-size: 15px;
  color: #111827;
}

.gd-offers-footer-text p {
  margin: 0;
  font-size: 14px;
  color: #4b5563;
}
...
<body>
    ...
    <header id="main-header"><div class="nav-logo"> <a href="https://6ixdevelopers.com/"><img id="logo" src="media/logo/new-logo.png" width="125px"></a>  <img style="position:relative; top:-4px; left:10px" id="ca-logo" src="media/canadian.png" width="40px"></div><div class="nav-li"> <a class="cta" href="contact-us"><i style="color:white; font-size:18px; position:relative; top:2px;" class="fa fa-envelope"></i> &nbsp;Contact us</a> <a href="tel:18888087265"><i style="font-size:12px" class="fas fa-phone-alt"></i> 1 888-808-7265</a> <a href="about-us">About Us</a><div class="dropdown"> <a class="dropbtn" href="#">Services <i class="fa fa-caret-down"></i></a><div class="dropdown-content"> <a href="website-design-agency-toronto">Website Design</a><br> <a href="ppc-google-ads-management-toronto">Google Ads/PPC</a><br> <a href="social-media-marketing-agency-toronto">Social Media</a><br> <a href="seo-agency-toronto">SEO Services</a><br></div></div> <a href="https://6ixdevelopers.com/">Home</a></div> <nav class="topnav"> <a href="tel:18888087265"><i style="color:#ff6699; font-size: 23px; position:relative; top:-2px;" class="fas fa-phone-alt"></i></a> &nbsp; &nbsp; &nbsp; <a href="#" onclick="openNav()"> <svg width="30" height="26" id="icoOpen"> <path d="M0,5 30,5" stroke="white" stroke-width="4"/> <path d="M0,14 30,14" stroke="white" stroke-width="4"/> <path d="M0,23 30,23" stroke="white" stroke-width="4"/> </svg> </a> </nav><div class="clear"></div> </header><div id="sideNavigation" class="sidenav"> <a href="javascript:void(0)" class="closebtn" onclick="closeNav()">&times;</a> <a style="margin-top:20px" href="https://6ixdevelopers.com/">Home</a> <a href="#">Services &nbsp;<i class="fa fa-caret-down"></i></a> <a style="padding-left:50px"href="website-design-agency-toronto">Website Design</a> <a style="padding-left:50px" href="ppc-google-ads-management-toronto">Google Ads/PPC</a> <a style="padding-left:50px" href="social-media-marketing-agency-toronto">Social Media</a> <a style="padding-left:50px" href="seo-agency-toronto">SEO Services</a> <a href="about-us">About Us</a> <a href="contact-us">Contact Us</a> <a class="cta" href="tel:18888087265"><i style="color:white; font-size:12px" class="fas fa-phone-alt"></i> &nbsp;1 888-808-7265</a></div><div class="clear"></div><div class="header-section">
        ...
        <div class="header-image"><h1>#1 Rated Google Ads Agency in Toronto</h1>
        
        <h3>We Count Leads, Not Clicks</h3>
        
        <p>Elevate your marketing through industry-leading:<br> <span class="typing-effect" typing-speed="70" typing-delay="1000" words="Google Ads Management,Target Qualified Leads,Advance Demographic Targeting,Click Fraud Protection"></span></p><br>
        
        <a class="btn btn-header tt-button" href="#pricing">Pricing</a>  &nbsp;<span class="tt-btn"></span> <a class="btn btn-header tt-button" href="#check-eligibility">Start Advertising Today</a> 
        
        
        </div></div>
    
    <!-- CHATGPT CODE -->
    <section class="gd-offers-wrapper">
  <div class="gd-offers">
    <div class="gd-offers-header">
      <h2>Choose a sign-up offer to jumpstart your first Google Ads campaign</h2>
      <p>
        Select an offer that fits your monthly Google Ads budget.
        <a href="https://ads.google.com/intl/en_au/home/terms-and-conditions/incentives/?bc=AU&bid=s1500g1000%7Cib%3A9279461727%7C&_gl=1*18xt0ga*_ga*MTU5MTM2NjQzOC4xNzY1NDY5MTIz*_ga_V9K47ZG8NP*czE3NjU0NzMxODEkbzIkZzAkdDE3NjU0NzMxODEkajYwJGwwJGgw*_ga_J51Y85KVRZ*czE3NjU0NzMxODEkbzIkZzAkdDE3NjU0NzMxODEkajYwJGwwJGgw" class="gd-offers-link">Terms and conditions apply.</a>
      </p>
    </div>

    <div class="gd-offers-cards">
      <!-- OFFER A -->
      <label class="gd-offer-card" data-offer="600">
        <input type="radio" name="gd-offer" value="600" class="gd-offer-input">
        <div class="gd-offer-inner">
          <div class="gd-offer-tag">Offer A</div>
          <div class="gd-offer-price">$600</div>
          <div class="gd-offer-subtitle">in ad credit</div>
          <p class="gd-offer-text">
            Spend $600 with Google Ads in the first 60 days to unlock the credit.
          </p>
          <span class="gd-offer-check" aria-hidden="true">
            <!-- check icon -->
      ...
      <!-- OFFER B (default selected) -->
      <label class="gd-offer-card gd-offer-card--selected" data-offer="1200">
        <input type="radio" name="gd-offer" value="1200" class="gd-offer-input" checked>
        <div class="gd-offer-inner">
          <div class="gd-offer-tag">Offer B</div>
          <div class="gd-offer-price">$1200</div>
          <div class="gd-offer-subtitle">in ad credit</div>
          <p class="gd-offer-text">
            Spend $1800 with Google Ads in the first 60 days to unlock the credit.
          </p>
          <span class="gd-offer-check" aria-hidden="true">
            <svg viewBox="0 0 24 24">
              <polyline points="4 13 9 18 20 6" />
      ...
      <!-- OFFER C -->
      <label class="gd-offer-card" data-offer="1800">
        <input type="radio" name="gd-offer" value="1800" class="gd-offer-input">
        <div class="gd-offer-inner">
          <div class="gd-offer-tag">Offer C</div>
          <div class="gd-offer-price">$1800</div>
          <div class="gd-offer-subtitle">in ad credit</div>
          <p class="gd-offer-text">
            Spend $3600 with Google Ads in the first 60 days to unlock the credit.
          </p>
          <span class="gd-offer-check" aria-hidden="true">
            <svg viewBox="0 0 24 24">
              <polyline points="4 13 9 18 20 6" />
    ...
    <div class="gd-offers-footer">
      <div class="gd-offers-footer-text">
        <h3>Claim your offer</h3>
        <p>
          Fill out the form below to apply the offer to your account.
        </p>
      </div>
      <button type="button" class="gd-offers-cta" id="gd-claim-offer-btn">
        Claim Now
      </button>
    </div>
  </div>
...
<!-- CHATGTP CODE end -->
 ...
 <div id="check-eligibility" class="body-section-5"><h2>Check Your Eligibility To Get Up To $1800 In Google Ads Credit</h2>
        ...
        <div class="email-field">
            ...
            <div class="email-field">
            <label>Choose a sign-up offer <span style="color:red">*</span></label>
            <select id="inquiry-typed" name="inquiry-typed">
                <option selected value="more-calls">$600 in ad credit (Spend $600 with Google Ads in the first 60 days to unlock the credit)</option>
                <option value="more-visits">$1200 in ad credit (Spend $1800 with Google Ads in the first 60 days to unlock the credit)</option>
                <option value="ecommerce">$1800 in ad credit (Spend $3600 with Google Ads in the first 60 days to unlock the credit)</option>
            </select>
        </div>
        ...
        <input style="border:none; padding-top: 16px;padding-bottom:16px; padding-left: 36px; padding-right:36px;" class="cst-btn submit-btn btn" type="submit" id="submit" name="submit" value="Check Eligibility"></form>
...
<p>Gain the competitive edge you deserve – partner with us for unparallel Google Ads/PPC management in Toronto!</p></div></div>
    
    <div class="case-study-section">
        
        <h2>Google Ads Client Success</h2>
        <h5>Experienced Team of Google Ads Experts, Focusing on the Tangible Results from Google Ads that makes the difference.</h5>
        <p>&nbsp;</p>
    ...
    <div class="campaign-audit-section">
        ...
        <div class="cas-heading"><h2>Comprehensive Google Ads Audit</h2></div>
        ...
        <div class="cas-row-1">
            ...
            <div class="casr1-text">
                ...
                <p class="casr1-cta">Don't leave the success of your campaigns to chance – schedule your Google Ads account audit today by booking a discovery call with one of our experts. Click below to get started and take control of your campaigns now!</p>
                
                <a href="#check-eligibility" class="cst-btn btn">Request Google Ads Account Audit</a>
        ...
        <div class="cas-row-2">
            ...
            <div class="casr2-text">
                ...
                <p class="casr2-cta">Don't settle for mediocre campaign performance – schedule your Google Ads account audit today by booking a discovery call with one of our specialists. Click below to get started and unlock the full potential of your Google Ads campaigns!</p>
      ...
      <!--  <div id="get-audit" class="body-section-5" style="padding: 70px">
        ...
        <h2>Google Ads Audit Request</h2>
        ...
        <form method="post" id="audit-form" class="qm-form">
            ...
            <div class="name-feild">
                ...
                <select id="audit-inquiry-type" name="audit-inquiry-type" >
                    ...
                    <option value="client">Someone Else's Business</option>
                </select>
            </div>
            <div class="email-field">
            ...
            <section id="r3-2" class="section-slide-form-3 form-section-three">
                <div class="inside-error-message-3 error-message"></div>
            <div class="name-feild"> 
            <label>Google Ads Marketing Objective  <span style="color:red">*</span></label>
                <input type="text" class="name" id="audit-goals" name="audit-goals" placeholder="E.g, 30 leads/month, etc.">
            </div>
            <div class="email-field"> 
            ...
            <div class="email-field">
                <label>Your Unique selling proposition</label>
            ...
            <section id="r5-2" class="section-slide-form-3 form-section-five">
                <div class="inside-error-message-3 error-message"></div>
            <div class="name-feild">
                <label>Current number of leads/Month</label>
                <input type="number" id="audit-current-leads" name="audit-current-leads">
            </div>
            <div class="email-field">
                <label>Desired number of leads/Month</label>
            ...
            <section id="r6-2" class="section-slide-form-3 form-section-six">
                <div class="inside-error-message-3 error-message"></div>
            <div class="name-feild"> 
            <label>Your Current Monthly Ad spend  <span style="color:red">*</span></label>
...
<button class="next-3" id="next">NEXT</button>
    ...
    <div class="campaign-audit-section-2">
        ...
        <h2 style="color:white">What's included in our comprehensive Google Ads Account Audit</h2>
        <div class="divider"></div>
        ...
        <div class="cas2-cta-section">
            ...
            Curious about where you stand against competitors in the Google Ads arena? Our complementary Google Ads audit is here to provide answers. Discover how your account measures up, pinpoint areas for improvement, and receive actionable recommendations to optimize your Google Ads performance.<br>
<br>Ready to take your Google Ads account to the next level? Click below to get started and unlock the insights you need to thrive in the competitive digital landscape.
    ...
    <div class="bs7-rows bs7-row-1"><div class="row-columns row-column-1 bs7-row-1-img"></div><div class="row-columns row-column-2"><a  href="##check-eligibility"><h2>Maximize Your ROI with Strategic Google Ads Management</h2></a><p>
...
</div></div><div class="bs7-rows bs7-row-2"><div class="row-columns row-column-1 bs7-row-2-img mob-row"></div><div class="row-columns row-column-2"><a  href="#check-eligibility"><h2>Stay Ahead of Evolving Search Trends with Our Google Ads Certified Team</h2></a><p>In today's dynamic digital landscape, the way people search for services is constantly evolving. That's why our Google Ads certified team of specialists is dedicated to keeping your advertising strategy finely tuned to capture maximum leads.</p>
...
</div><div class="row-columns row-column-1 bs7-row-2-img desk-row"></div></div><div class="bs7-rows bs7-row-3"><div class="row-columns row-column-1 bs7-row-3-img"></div><div class="row-columns row-column-2"><a  href="#check-eligibility"><h2>Unlock Precision Targeting with GEO Google Ads Marketing Strategy</h2></a><p>At 6ix Developers, we prioritize precision in targeting by implementing a GEO marketing strategy for your Google Adword campaigns. By focusing on specific geographic locations, we ensure that your ads are shown only to users within your target audience, maximizing relevance and driving desired actions on your website.</p>
...
</div></div><div class="bs7-rows bs7-row-4"><div class="row-columns row-column-1 bs7-row-4-img mob-row"></div><div class="row-columns row-column-2"><a  href="#check-eligibility"><h2>Don't Underestimate the Impact of Website Speed on Your Google Ad Performance</h2></a><p>The speed of your website's landing page is often underestimated, yet it plays a crucial role in determining your cost per click on Google Ads. A slow-loading page can result in higher costs per click compared to your competitors, even if they rank below you.</p>
...
</div><div class="row-columns row-column-1 bs7-row-4-img desk-row"></div></div></div><div class="body-section-2"><div class="bs2-section"><h2>Get the Results That Matter to You</h2><div class="blurb-content"> <a href="#check-eligibility"><div class="blurb blurb-1"><div class="blurb-image"><img src="media/icons/website-visit.jpg"></div><div class="blurb-text"><h3>Get More Qualified Leads</h3><p>Generate high intent, qualified leads by ranking #1 on Google at the right time.</p></div></div></a><div class="clear-mb2"></div><a href="#check-eligibility"><div class="blurb blurb-2"><div class="blurb-image"><img src="media/icons/more-call.jpg"></div><div class="blurb-text"><h3>More Phone Calls</h3><p>Increase qualified call volume using Google Ads high-intent targeting.</p></div></div></a><div class="clear-mbb"></div> <a href="#check-eligibility"><div class="blurb blurb-3"><div class="blurb-image"><img src="media/icons/store-visit.jpg"></div><div class="blurb-text"><h3>Transparency</h3><p>Understand what's working with quantifiable Google Ads reports.</p></div></div></a></div></div><a class="btn" href="#check-eligibility">Start Advertising</a></div><div class="body-section-3"><h2>How It Works</h2><div class="bs3-section"><div class="bs2-row-1 bs2-row"><h4>What Do We Need From You?</h4><div class="divider"></div><p>To begin creating your Google Ads campaign, we’ll need a few things from you first.</p><h5 style="margin-top:20px;">Manager Access</h5><p>We will access your Google Ads account through Manager Level Access. We do not need your account login information and you are in full control of your account.</p><h5 style="margin-top:15px;">Monthly Budget</h5><p>Agreed upon monthly budget. Your Google Ads specialist will conduct industry competitors research to determine the optimal monthly budget for your business.</p><h5 style="margin-top:15px;">Website URL</h5><p>We will need your website URL to setup conversion tracking. </p></div><div class="bs2-row-2 bs2-row"><h4>On-boarding Process</h4><div class="divider"></div><p style="margin-top:15px;"><h5 style="margin-top:20px;">Step 1.</h5> Our Google Ads specialists will conduct a comprehensive market research to develop an effective strategy.</p><p style="margin-top:15px;"><h5 style="margin-top:20px;">Step 2.</h5> Curate a list of lead generating keywords for Google Ads that suits the monthly budget and send it to you for your approval.</p><p style="margin-top:15px;"><h5 style="margin-top:20px;">Step 3.</h5> We build an effective Google Ads campaign structure with supporting Ad Groups and send it to you for approval.</p><p style="margin-top:15px;"><h5 style="margin-top:20px;">Step 4.</h5> We develop creative ad copy e.g. headlines and descriptions, call our extensions and send it to you for approval.</p><p style="margin-top:15px;"><h5 style="margin-top:20px;">Step 5.</h5> We set up Google Ads Conversions Tracking, Integrate Advance Click Fraud Protection and create your Google Ads Reporting Dashboard where you're able to see quantifiable results.</p></div><div class="clear"></div></div><div class="body-section-8"><div class="bs8-first-section bs-8-section"><div class="bs8-row bs8-first-row"><div class="bs-8-image"><img src="media/icons/No-Contract-ppc-management-agency.png"></div><div class="bs-8-text"><h4>No Contract</h4><p>Performance based month to month Google Ads management.</p></div></div><div class="bs8-row bs8-first-row"><div class="bs-8-image"><img src="media/icons/Low-one-time-setup-fee-ppc-management-agency.png"></div><div class="bs-8-text"><h4>Get up to $1800 Ad Credit</h4><p>Receive a credit of up to $1800 by starting Google Ads with us. <a href="#check-eligibility">Check your eligibility</a></p></div></div></div><div class="bs8-second-section bs-8-section"><div class="bs8-row bs8-second-row"><div class="bs-8-image"><img src="media/icons/Flat-rate-monthly-fee-google-ppc-agency-toronto.png"></div><div class="bs-8-text"><h4>Flat Rate Monthly Fee</h4><p>You pay the agreed upon monthly management fee - no surprises.</p></div></div><div class="bs8-row bs8-second-row"><div class="bs-8-image"><img src="media/icons/Transparent-google-adwords-agency-toronto.png"></div><div class="bs-8-text"><h4>Transparency</h4><p>We work on your Google Ads Account & you pay Google Ads directly for the PPC.</p></div></div></div></div></div><div class="body-section-1" id="pricing"><div class="boxes"><h2>Grow Your Business With Google Ads</h2><p style="margin-top:15px;">No matter the size of your business, a strong Google Ads campaign is essential to capture qualified leads.</p><p>Google Ads is ideal for businesses that are looking to capture high-intent, qualified leads.</p><div class="bs1-section"><div class="box-cal">
    
    <div class="calculator"><h4>Find out your monthly management cost</h4><form method="get" id="calculate-management" name="calculate-management"> <input type="text" class="cal-field" id="cal-field" name="cal-field" placeholder="Enter your monthly Google Ads budget">
    
    <div class="form-button"> 
    
    <input style="border:none; padding-top: 16px;padding-bottom:16px; padding-left: 36px; padding-right:36px; margin-bottom:10px;" type="submit" id="cal-value" class="btn btn-form" value="Calculate Now"><br>
            <a class="btn btn-header" href="#check-eligibility">Talk to PPC Expert</a>
    
    </div><p style="margin-top:15px;" class="cal-result"></p></form></div><div class="box-cal-txt"><h4>One Time Setup Fee:</h4><p>Onboarding Fee: $1500. New Google Ads account pay only $999.</p><p>&nbsp;</p><h4>Our Management Fee:</h4><p>New Google Ads accounts pay $0 for the first 2 Months. We want to build trust by showing the results before taking your money. After the first 2 months, our management fee is $799 per month or 15% of monthly Google Ads budget, whichever is greater. $0 for 2 months apply on new Google Ads accounts only.</p></div></div><div class="box-content"><div class="box box-1"><div class="box-heading bh2"><h3>What's Included</h3></div><div class="box-txt"><div class="bt-section"><div class="box-row-firsthalf">Platforms</div><div class="box-row-secondthalf">Google Ads</div></div><div class="clear"></div><div class="bt-section"><div class="box-row-firsthalf">Keyword Research</div><div class="box-row-secondthalf">Yes</div></div><div class="clear"></div><div class="bt-section"><div class="box-row-firsthalf">Competitor Research</div><div class="box-row-secondthalf">Yes</div></div><div class="clear"></div><div class="bt-section"><div class="box-row-firsthalf">Review Calls</div><div class="box-row-secondthalf">Monthly</div></div><div class="clear"></div><div class="bt-section"><div class="box-row-firsthalf">Account Access</div><div class="box-row-secondthalf">Manager</div></div><div class="clear"></div><div class="bt-section"><div class="box-row-firsthalf">Reporting Dashboard</div><div class="box-row-secondthalf">Yes</div></div><div class="clear"></div><div class="bt-section"><div class="box-row-firsthalf">Google Analytics</div><div class="box-row-secondthalf">Yes</div></div><div class="clear"></div><div class="bt-section"><div class="box-row-firsthalf">Goal Setting</div><div class="box-row-secondthalf">Yes</div></div><div class="bt-section"><div class="box-row-firsthalf">Click Fraud Protection</div><div class="box-row-secondthalf">Yes</div></div><div class="clear"></div><div class="bt-section"><div class="box-row-firsthalf">Call & Conversion Tracking</div><div class="box-row-secondthalf">Yes</div></div><div class="clear"></div><div class="bt-section"><div class="box-row-firsthalf">Remarketing & Display Ads</div><div class="box-row-secondthalf">Yes</div></div><div class="clear"></div></div></div></div></div></div></div>
    
 <!--  <div style="padding-top: 10px;" id="get-quote" class="body-section-5">
    <h2>Book You Google Ads Consultation</h2>
    ...
    <form method="post" id="quote-form" class="qm-form">
        ...
        <section id="r1-1" class="section-slide-form-2 form-section-one active2">
            ...
            <div class="email-field">
            <label>Choose your marketing goal <span style="color:red">*</span></label>
...
<button class="next-2" id="next">NEXT</button>
  ...
  (function () {
    const cards = document.querySelectorAll(".gd-offer-card");
    const claimBtn = document.getElementById("gd-claim-offer-btn");

    // Card selection logic
    cards.forEach((card) => {
      card.addEventListener("click", () => {
        cards.forEach((c) => c.classList.remove("gd-offer-card--selected"));
        card.classList.add("gd-offer-card--selected");

        const input = card.querySelector(".gd-offer-input");
        if (input) {
          input.checked = true;
        }
      });
    ...
    if (claimBtn) {
      claimBtn.addEventListener("click", () => {
        ...
        if (target) {
          ...
          // Fallback: jump using hash if element is missing for some reason
          window.location.href =
            "https://6ixdevelopers.com/ppc-google-ads-management-toronto#check-eligibility";
        }
      });
    }
  })();
